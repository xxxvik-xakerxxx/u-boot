...
static void fixture_init(struct fixture *fixture, bool v2, bool use_rmem)
{
	...
	memory_reg[3] = cpu_to_fdt32(0x200000);
	...
	if (use_rmem) {
		...
		reg[0] = cpu_to_fdt32(fixture->base >> 32);
		reg[1] = cpu_to_fdt32(fixture->base);
		reg[2] = cpu_to_fdt32(0);
		reg[3] = cpu_to_fdt32(fixture->descriptor_size);
		require(!fdt_setprop(fixture->fdt, node, "reg", reg, sizeof(reg)),
			"set LK tag reservation");
	} else {
		ret = fdt_add_mem_rsv(fixture->fdt, fixture->base,
...
static bool fixture_range_allowed(u64 base, u32 size, void *context)
{
	...
	return fixture->allow_range && base == fixture->base &&
	       size == fixture->descriptor_size;
}
